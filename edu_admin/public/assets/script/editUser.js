$(document).ready(function () {
    ...
    const subCategoryOptions = $("#sub_category_id option")
        .map(function () {
            return {
    ...
    const topicOptions = $("#topic_id option")
        .map(function () {
            return {
                value: $(this).attr("value"),
                text: $(this).text(),
    ...
    const stateOptions = $("#state option")
        .map(function () {
            return {
                value: $(this).attr("value"),
                text: $(this).text(),
    ...
    $("#editUserForm").on("submit", function (e) {
        ...
        checkUserType(() => {
            ...
            var is_verify = $("#switchIsVerify").length && $("#switchIsVerify").prop("checked") === true ? 1 : 0;
    ...
    $("#addDiamondsForm").on("submit", function (e) {
        e.preventDefault();
        checkUserType(() => {
            var formId = "#addDiamondsForm";
            var url = `${domainUrl}addDiamondsToUserWallet_FromAdmin`;
            var formdata = collectFormData(formId);
            showFormSpinner(formId);
            try {
                doAjax(url, formdata).then(function (response) {
