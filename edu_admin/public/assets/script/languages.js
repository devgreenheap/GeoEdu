$(document).ready(function () {
    ...
    $("#addLanguageForm").on("submit", function (e) {
        e.preventDefault();
            checkUserType(() => {
                ...
                var localizedTitle = $.trim($("#localized_title").val() || "");

                $("#code").val(code);
                $("#title").val(title);
                $("#localized_title").val(localizedTitle);

                if (!code || !title || !localizedTitle) {
                    showErrorToast("Code, title and localized title are required");
                    return;
                }
    ...
    $("#editLanguageForm").on("submit", function (e) {
        e.preventDefault();
            checkUserType(() => {
                var editCode = $.trim($("#edit_code").val() || "");
                var editTitle = $.trim($("#edit_title").val() || "");
                var editLocalizedTitle = $.trim($("#edit_localized_title").val() || "");

                $("#edit_code").val(editCode);
                $("#edit_title").val(editTitle);
                $("#edit_localized_title").val(editLocalizedTitle);

                if (!editCode || !editTitle || !editLocalizedTitle) {
                    showErrorToast("Code, title and localized title are required");
                    return;
                }
