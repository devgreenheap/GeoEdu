...
class CustomImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rawUrl = image ?? '';
    final isSvg = rawUrl.toLowerCase().contains('.svg');
    final isAsset = rawUrl.startsWith('assets/');
    final double smoothing = cornerSmoothing ?? 0;
      ...
      if (isSvg) {
        childWidget = SizedBox(
          ...
          child: ClipSmoothRect(
            ...
            child: isAsset
                ? SvgPicture.asset(rawUrl, fit: fit ?? BoxFit.contain)
                : SvgPicture.network(rawUrl, fit: fit ?? BoxFit.contain),
          ),
        );
      } else if (isAsset) {
        childWidget = SizedBox(
          ...
          child: ClipSmoothRect(
            radius: SmoothBorderRadius(
                cornerRadius: radius, cornerSmoothing: smoothing),
            child: Image.asset(rawUrl, fit: fit ?? BoxFit.cover),
          ),
        );
      }
      if (childWidget != null) {
        if (onTap != null) {
          return GestureDetector(onTap: onTap, child: childWidget);
        }
        return childWidget;
      }
    }

    // A malformed URL (bad encoding, stray characters from an old/corrupt
    // record, etc.) makes CachedNetworkImage throw synchronously instead of
    // routing to its errorWidget callback — Flutter then silently renders
    // that as a blank box in release builds (no crash, no error text, just
    // an unresponsive gap where the card should be). Validating up front
    // means a bad URL degrades to the normal initials/placeholder fallback
    // instead of taking the whole card down.
    final isValidUrl = rawUrl.isNotEmpty && (Uri.tryParse(rawUrl)?.hasScheme ?? false);
    String imageUrl = isValidUrl ? rawUrl : '';
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: isShowPlaceHolder
          ...
          : Text(
              ((fullName != null && fullName!.isNotEmpty) ? fullName![0] : AppRes.appName[0]).toUpperCase(),
