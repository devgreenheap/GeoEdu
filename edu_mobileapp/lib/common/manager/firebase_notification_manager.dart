...
import 'package:geoedu/common/manager/session_manager.dart' show SessionManager;
...
class FirebaseNotificationManager {
  ...
  Future<void> handleNotification(String payload) async {
    ...
    switch (dataType) {
      ...
      case 'audio_call':
        controller.selectedPageIndex.value = 2;
        await _handleAudioCallNotification(dataString);
        break;
      case 'agent_users':
        controller.selectedPageIndex.value = 5;
  ...
  Future<void> _handleAudioCallNotification(String dataString) async {
    try {
      // The audio call notification triggers the incoming call listener
      // in AudioCallListController which is already active on tab 2.
      // If the controller is registered, it will handle the incoming call overlay.
      if (!Get.isRegistered<AudioCallListController>()) {
        Get.put(AudioCallListController());
      }
    } catch (e) {
      Loggers.error('Failed to handle audio call notification: $e');
    }
  }
