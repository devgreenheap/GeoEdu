...
class Setting {
  ...
  factory Setting.fromJson(Map<String, dynamic> json) => Setting(
        ...
        battleDurationMinutes: json["battle_duration_minutes"],
  ...
  Map<String, dynamic> toJson() => {
        ...
        "battle_duration_minutes": battleDurationMinutes,
...
extension SettingGiftExt on Setting? {
  List<Gift> get allGiftsWithPen {
    if (this == null) return [Gift.penGift];
...
class Gift {
  ...
  static final Gift penGift = Gift(
    id: 18,
    title: 'Pen',
    giftCategoryId: 2,
    coinPrice: 10,
  ...
  /// Returns clean display name (e.g. 'Pen', 'Celebration', 'Chocolate')
  String get displayName {
    if (title != null && title!.trim().isNotEmpty) {
      String clean = title!.trim();
      clean = clean.replaceAll(
          RegExp(r'\.(svg|gif|png|jpg|jpeg|mp4)$', caseSensitive: false), '');
      clean = clean
          .replaceAll(RegExp(r'\s*animation\s*', caseSensitive: false), ' ')
          .trim();
      if (clean.isNotEmpty) return clean;
    }
    if (image != null && image!.isNotEmpty) {
      final filename = image!.split('/').last.split('\\').last;
      String name = filename.replaceAll(
          RegExp(r'\.(svg|gif|png|jpg|jpeg|mp4)$', caseSensitive: false), '');
      name = name.replaceAll(RegExp(r'^\d+_Shortzz_', caseSensitive: false), '');
      name = name.replaceAll(
          RegExp(r'^(Shortzz_|iconshome-|mxjfiles-|placidplace-)',
              caseSensitive: false),
          '');
      name = name.replaceAll(RegExp(r'[-_]', caseSensitive: false), ' ');
      name = name.replaceAll(RegExp(r'\s*\(\d+\)\s*'), '');
      name = name.replaceAll(
          RegExp(r'\s*animation\s*', caseSensitive: false), ' ');
      name = name.replaceAll(
          RegExp(r'\s*lottie\s*', caseSensitive: false), ' ');
      name = name.replaceAll(
          RegExp(r'\s*gif json svg\s*', caseSensitive: false), ' ');
  ...
  /// What plays room-wide when this gift is sent — the dedicated animation
  /// if one was uploaded, otherwise the static thumbnail image.
...
class UserLevel {
  ...
  factory UserLevel.fromJson(Map<String, dynamic> json) => UserLevel(
        id: json["id"],
        level: json["level"],
        coinsCollection: json["coins_collection"] ?? 0,
        createdAt: json["created_at"] == null
            ? null
            : DateTime.parse(json["created_at"]),
        updatedAt: json["updated_at"] == null
