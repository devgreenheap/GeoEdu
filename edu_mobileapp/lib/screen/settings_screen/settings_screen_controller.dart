...
class SettingsScreenController extends BaseController {
  ...
  // Dynamic counts for Transactions section
  RxInt diamondCount = 0.obs;
  RxInt starCount = 0.obs;
  RxInt myEffectsCount = 0.obs;
  ...
  void initData() {
    ...
    // Star count from user model
    starCount.value = myUser.value?.coinWallet?.toInt() ?? 0;
  ...
  Future<void> fetchDiamondCount() async {
    try {
      final wallet = await GiftWalletService.instance.fetchMyDiamondWallet();
      diamondCount.value = wallet.data?.diamondBalance ?? 0;
    } catch (e) {
      Loggers.error('fetchDiamondCount error: $e');
    }
  }

  Future<void> fetchMyEffectsCount() async {
    try {
      final effects = await GiftWalletService.instance.fetchMyEntryEffects();
      myEffectsCount.value = effects.length;
    } catch (e) {
      Loggers.error('fetchMyEffectsCount error: $e');
    }
  }
  ...
  Future<void> requestDisableScreenshot(String reason) async {
    showLoader();
    try {
      final result =
          await GiftWalletService.instance.requestScreenshotDisable(reason: reason);
      if (result.status == true) {
        myUser.value?.disableScreenshotStatus = 1; // pending
        myUser.update((user) {});
        // Refresh user data
        ...
        stopLoader();
        showSnackBar(result.message ?? 'Failed to submit request');
      }
    } catch (e) {
      stopLoader();
  ...
  void onDeleteAccount() {
    Get.bottomSheet(ConfirmationSheet(
        ...
        actionColor: ColorRes.primaryColor,
        boldWhiteDescription: true));
  }
