...
import '../../utilities/text_style_custom.dart' show TextStyleCustom;
...
class FollowersScreen extends StatelessWidget {
  ...
  const FollowersScreen({super.key, this.user});

  @override
  Widget build(BuildContext context) {
    final controller = Get.put(
      FollowFollowingScreenController(
          FollowFollowingType.follower, user, context),
      tag: '${user?.id}_followers',
    );

    return Scaffold(
      appBar: AppBar(
        title: Text(LKey.followers.tr),
        // centerTitle: true,
        flexibleSpace: Container(
          decoration: const BoxDecoration(
            image: DecorationImage(
              image: AssetImage("assets/images/followers_appbar.png"),
              fit: BoxFit.cover,
            ),
          ),
        ),
      ...
      body: Obx(() {
        final isEmpty = controller.followers.isEmpty;
        final showLoader =
            controller.isFollowers.value && isEmpty;
        final showNoData =
            !controller.isFollowers.value && isEmpty;
        ...
        return Stack(
          children: [
            ...
            // Positioned.fill(child: Image.asset("assets/images/followers_bg.png",fit: BoxFit.cover,)),
            _buildUserList(
...
class _UserProfileTileState extends State<UserProfileTile> {
  ...
  Widget build(BuildContext context) {
    return LoadMoreWidget(
      ...
      child: Padding(
        padding: const EdgeInsets.only(left: 10.0, right: 10.0, bottom: 15.0,top: 15),
        child: Column(
          children: [
            Row(
              children: [
            ...
            CustomDivider(
              color: bgGrey(context).withOpacity(0.70),
              height: 0.5,
